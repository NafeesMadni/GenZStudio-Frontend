...
export default function TikTokVideoDownloader() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/tiktok-video-metadata`, {
        method: 'POST',
        headers: {
        ...
        <div className="space-y-6 p-6 bg-gradient-to-b from-sky-950/50 to-slate-900/50 rounded-xl border border-sky-400/30 backdrop-blur-sm">
          ...
          <div className="text-center space-y-3 mb-8">
            ...
            <p className="text-slate-400 text-xs md:text-sm lg:text-base max-w-xl mx-auto">
              Download your favorite TikTok videos in HD quality
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-3">
              <label htmlFor="video_url" className="flex items-center gap-2 text-slate-200 font-medium">
                <svg
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-lg bg-slate-700/50 text-white border-2 border-slate-600 focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:border-transparent transition-all duration-300 text-base backdrop-blur-sm"
                required
              />
            </div>
          ...
          <div className="mt-8">
            <div className="p-6 bg-gradient-to-b from-sky-950/50 to-slate-900/50 rounded-xl border border-sky-400/30 backdrop-blur-sm transition-all duration-300 flex items-center justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-slate-200 font-medium truncate">{videoTitle}</p>
                <p className="text-slate-400 text-sm truncate">{channelName}</p>
              </div>
              <button
