...
export default function LinkedinVideoDownloader() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/linkedin-video-metadata`, {
        method: 'POST',
        headers: {
      ...
      if (data.error) {
        ...
        setVideoTitle(data.data.videoTitle || 'Linkedin HD video')
        setChannelName(data.data.channelName || 'Linkedin User')
        setShowResult(true)
      }
    } catch (error) {
      console.error(error)
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-3">
              ...
              <input
                ...
                placeholder="https://www.linkedin.com/video/..."
                className="w-full px-4 py-2.5 md:py-3 text-sm md:text-base rounded-lg bg-gray-700/80 border border-gray-600 focus:ring-2 focus:ring-blue-500 focus:border-blue-600 placeholder-gray-400 transition-all duration-300"
                required
              />
            </div>
