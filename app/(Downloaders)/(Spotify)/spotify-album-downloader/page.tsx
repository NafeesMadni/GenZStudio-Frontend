...
export default function SpotifyAlbumDownloader() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/spotify-album-metadata`, {
        method: 'POST',
        headers: {
      ...
      if (data.error) {
        setError(data.message || 'Error fetching album information')
      } else {
  ...
  const handleDownloadZip = () => {
    ...
    const fileName = `${albumData.albumName} by ${albumData.albumArtists}`
    
    window.location.href = `/api/stream/zip?tracks_metadata=${encodeURIComponent(
      JSON.stringify(albumData.tracks_metadata)
        ...
        <div className="space-y-6 p-6 bg-gradient-to-b from-sky-950/50 to-slate-900/50 rounded-xl border border-sky-400/30 backdrop-blur-sm">
          <div className="text-center space-y-3 mb-8">
            ...
            <p className="text-slate-400 text-xs md:text-sm lg:text-base max-w-xl mx-auto">
              Download Album Tracks in High quality
            </p>
          </div>
        ...
        {showResult && albumData && (
          <div className="mt-8">
            <div className="p-6 bg-gradient-to-b from-sky-950/50 to-slate-900/50 rounded-xl border border-sky-400/30 backdrop-blur-sm transition-all duration-300">
              ...
              <div className="space-y-3">
                {albumData.tracks_metadata.map((track, index) => (
                  <div
                    key={index}
