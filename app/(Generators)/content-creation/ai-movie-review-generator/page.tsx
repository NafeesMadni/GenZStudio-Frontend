...
export default function AIMovieReviewGenerator() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/content-creation/ai-movie-review-generator`, {
        method: "POST",
        headers: {
      ...
      <div className="max-w-[785px] w-full px-4 py-8">
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-400">
            Generate detailed, insightful movie reviews with AI assistance
          </p>
        </div>
          ...
          <div className="mt-8 p-6 bg-gradient-to-b from-sky-950/50 to-slate-900/50 rounded-xl border border-sky-400/30 backdrop-blur-sm">
            ...
            <div className="prose prose-invert prose-lg max-w-none prose-headings:text-cyan-400 prose-a:text-blue-400 prose-strong:text-white prose-ul:text-gray-300 prose-ol:text-gray-300">
              <Markdown components={{
                ...
                ul: ({node, ...props}) => (
                  <ul className="list-dot pl-5 space-y-1 text-gray-300" {...props} />
                ),
                // Override default heading rendering if needed
                h1: ({node, ...props}) => (
